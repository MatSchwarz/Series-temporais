...
taxa de eficiência de desemprego até dezembro de 2002.
Foi utilizada a função window na série.
...
Série parece não apresentar componentes determinísticos na primeira diferença.
Dessa forma, estima-se o ARMA em primeira diferença (ARIMA(p,1,q)) 
sem intercepto ou tendência.

Visualização do FAC e FACP para estimar o processo.
(FAC e FACP do processo estacionarizado na primeira diferença).
...
Vamos testar as opções com a função auxiliar "arima.est.parallel" e p máximo igual a 4 e q máximo igual a 3.
...
Ordenando a tabela em função dos menores BIC.

```{r}
tabela_ordemBIC <- tabela [order(tabela$BIC, decreasing = FALSE),]
```

Não há muita diferença entre os modelos, com AIC e BIC bem próximos. Os modelos ARIMA(2,1,2) e ARIMA(4,1,3) parecem mais compatíveis com as análises FAC e FACP.

Após testar os dois, com erros quadráticos médios muito próximos, optamos pelo ARIMA(2,1,2) com valores de estatística t (utilizando o erro-padrão) altos para todos os coeficientes AR e MA.

```{r}
modelo = Arima(desemprego_eficiente_wd, order =c(2,1,2), include.constant = F, include.drift = F)
summary(modelo)
```
...
O p-valor alto para o teste de Ljung-Box indica pouca correlação nos erros, o que é desejável.
...
### Questão 6
Efeitos estimados das reformas de janeiro de 2003 a dezembro de 2006, estimativas pontuais e os intervalos
de predição dos efeitos.



### Questão 7
Hipóteses de identificação para a validade da abordagem anterior em recuperar os efeitos de interesse Análise de razoabilidade no contexto da reforma e justificativas, preocupações com a identificação e sugestões de modificações da abordagem/métodos alternativos que mitiguem essas preocupações.
